...
df2=pd.read_csv(r'C:/Users/User/Desktop/work/UCY/datasets_analyzed/IoT/analysis/#1/1sec/experiment no DoS Binary/METHOD B/new_evaluation_DOS_1.csv')




features=[len(df1)]
for n_features in range(0, len(features)):
    print(features[n_features])
    X=df1.iloc[:,1:len(df1.columns)]

    y = df1['Class']    #target column i.e price range
    from skfeature.function.similarity_based import fisher_score
    ranks=fisher_score.fisher_score(X.to_numpy(),y.to_numpy(),mode="rank")

    feat_importance=pd.Series(ranks,X.columns[0:len(X.columns)])
    print(feat_importance.nlargest(features[n_features]).index)
    #feat_importance.plot(kind="barh",color="teal")
    #plt.show()

    path_to_file="C:/Users/User/Desktop/work/UCY/datasets_analyzed/IoT/analysis/#1/1sec/experiment no DoS Binary/METHOD B/RESULTS/fisher_score/correlations&fisher="+str(features[n_features])+"/"
    ...
    f1=open(path_to_file+"README.txt","a")



    X_train=df1[feat_importance.nlargest(features[n_features]).index]
    X_test=df2[feat_importance.nlargest(features[n_features]).index]
    ...
    MCC_LR=metrics.matthews_corrcoef(y_test, y_pred)
    ...
    f.write("\nAUC:"+str(auc))
    f1.write("\n---------------------------------------- LOGSTIC REGRESSION ----------------------------------------------------------\n")
    f1.write("Best parameters: %s" % result.best_params_)
    ...
    ## Display the visualization of the Confusion Matrix.
    plt.show()
    #if (f1_score_KNN> max(best)):
        #best.append(f1_score_KNN)
        #number.append(i)

    #print("Best f1-score",best,number)
    ...
    #f1.write("\nNum of features: %s" % features[n_features])
    f1.write(str(confusion_matrix(y_test,y_pred_KNN)))
    ...
    classifier_SVM= SVC(kernel="poly",random_state=0,gamma=0.8,C=1)
    ...
    parameters = [{'C': [0.25, 0.5, 0.75, 1], 'kernel': ['linear'],'random_state':[0]},
                  {'C': [ 0.5, 0.75, 1], 'kernel': ['rbf',"poly"], 'gamma': [0.1, 0.2, 0.4, 0.5, 0.6, 0.8],'random_state':[0]},
                  ]
    ...
    classifier_DT = DecisionTreeClassifier(criterion = 'entropy', random_state = 0,min_samples_split=20,
    ...
    classifier_RF = RandomForestClassifier(n_estimators =400, criterion = 'entropy', random_state = 0, max_depth=30, min_samples_leaf=1,min_samples_split=2)
    ...
    classifier_XG = XGBClassifier(colsample_bytree=0.6, gamma=1,max_depth=3,min_child_weight=1,subsample=1.0,random_state=0)
